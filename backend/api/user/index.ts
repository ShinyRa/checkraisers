...
class UserAPI extends BaseAPI {
    ...
    public updateProfile = async(user: Partial<User>): Promise<Response> => {
        try{
            ...
            if(userPresent){
                const res = await this.db.findOneAndUpdate({email: user.email}, {$set: user}, { upsert: false, returnDocument: 'after' })
                return res
        ...
        try{
            ...
            if(!userPresent){
                ...
                return res.acknowledged 
                    ? this.httpResponse(HttpStatusCode.SUCCESS, {error: "Created new user"})
    ...
    public login = async(user: Record<User['email'], User['password']>): Promise<Response> => {
        try{
            const profile = await this.db.findOne({email: user.email, password: this.hash(user.password)}).then((result) =>{
                return result ? result : false
            });
            return profile 
